...
public final class Geocoder
{
     ...
     */
    public List<GeoAddress> getFromLocation(double latitude, double longitude,
                                         int maxResults) throws IOException, LimitExceededException
    {
        ...
        final List<GeoAddress> results = new ArrayList<GeoAddress>();
     ...
     *             if the network is unavailable or any other I/O problem occurs
     */
    public List<GeoAddress> getFromLocationName(String locationName, int maxResults)
    ...
    private void parseJson(List<GeoAddress> address, int maxResults, byte[] data)
        ...
        try {
            ...
            if (status.equals(STATUS_OK)) {
                ...
                for (int i = 0; i < maxResults && i < a.length(); i++) {
                    final GeoAddress current = new GeoAddress();
                    final JSONObject item = a.getJSONObject(i);

                    current.setFeatureName(item.getString("formatted_address"));
                    JSONObject geometry = item.getJSONObject("geometry");
                    final JSONObject location = geometry
                            .getJSONObject("location");
                    current.setLatitude(location.getDouble("lat"));
                    current.setLongitude(location.getDouble("lng"));
                    if (geometry.has("bounds")) {
                        JSONObject viewport = geometry.getJSONObject("bounds");
                        LatLng northEast = new LatLng(viewport.getJSONObject("northeast").getDouble("lat"),
                                viewport.getJSONObject("northeast").getDouble("lng"));
                        current.setNorthEast(northEast);
                        LatLng southWest = new LatLng(viewport.getJSONObject("southwest").getDouble("lat"),
                                viewport.getJSONObject("southwest").getDouble("lng"));
                        current.setSouthWest(southWest);
                    }
                    address.add(current);
                }
